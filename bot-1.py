...
if not BOT_TOKEN:
    raise RuntimeError("Переменная окружения BOT_TOKEN не задана. Установи её перед запуском.")
...
async def start(message: types.Message):
    ...
    WEBAPP_URL = "https://tapix-3k3q.onrender.com?v=12"
...
async def api_handler(request):
    ...
    if action == 'transfer':
        ...
        cursor.execute("SELECT id, telegram_id FROM users WHERE username = ?", (to_username,))
    ...
    if action == 'get_top':
        return web.json_response({'success': True, 'top': top_cache['data']})
    ...
    if action == 'buy_boost':
        ...
        now = datetime.now()
        cooldown_raw = user['boost_cooldown']
        if cooldown_raw:
            try:
                cooldown_dt = datetime.fromisoformat(cooldown_raw)
            except (TypeError, ValueError):
                cooldown_dt = None

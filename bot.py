...
async def lottery_loop():
    while True:
        try:
            ...
            inner_hash = hashlib.sha256(seed.encode()).hexdigest()   # определяет результат, скрыт до конца
            display_hash = hashlib.sha256(inner_hash.encode()).hexdigest()  # показываем игрокам
            async with pool.acquire() as conn:
            ...
            lottery_state.update({
                ...
                'hash': display_hash,        # показывается игрокам (не раскрывает результат)
                'inner_hash': inner_hash,    # скрыт до конца раунда
                'seed': seed,
                'result': None
            })
            await asyncio.sleep(LOTTERY_BET_SECONDS)
            # Проверяем: если поставила только одна сторона — возвращаем ставки, не начисляем
            async with pool.acquire() as conn:
            ...
            if len(sides_with_bets) < 2:
                # Возвращаем ставки и закрываем раунд без победителя
                async with pool.acquire() as conn:
                    async with conn.transaction():
                        for r in rows:
                            await update_balance_and_xp(conn, r['telegram_id'], float(r['amt']))
                        await conn.execute(
                            "UPDATE lottery_rounds SET end_time=now(),status='cancelled' WHERE id=$1", rid
                        )
                lottery_state['phase'] = 'resolving'
                lottery_state['phase_end'] = time.time() + LOTTERY_REVEAL_SECONDS
                lottery_state['result'] = None
...
async def api_handler(request):
    ...
    # ===== LADDER (ЛЕСЕНКА) =====
    LADDER_MULTIPLIERS = [1.3, 1.6, 2.0, 2.5, 3.5, 5.0, 7.0, 10.0, 15.0, 25.0]
    LADDER_CELLS = 6
    ...
    if action == 'ladder_step':
        try:
            ...
            bombs = int(data.get('bombs', 2))
        except: return web.json_response({'success': False, 'error': 'Некорректные параметры'})
        if not 1 <= bombs <= 5: return web.json_response({'success': False, 'error': 'Бомб от 1 до 5'})
        if not 0 <= cell < LADDER_CELLS: return web.json_response({'success': False, 'error': 'Некорректная клетка'})
        # Generate bomb positions server-side
        bomb_positions = set()
        ...
        if is_bomb:
            # Deduct bet on first step only; client already deducted
            await conn.execute(
            ...
            next_step = step + 1
            complete = next_step >= len(LADDER_MULTIPLIERS)
            if complete:
                coef = LADDER_MULTIPLIERS[-1]
                win = bet * coef
                async with pool.acquire() as conn:
                    new_bal = await update_balance_and_xp(conn, tid, win)
                    await conn.execute(
                ...
                return web.json_response({'success': True, 'result': 'safe', 'bombs': bomb_list, 'complete': True, 'win_amount': win, 'new_balance': new_bal})
    ...
    if action == 'ladder_cashout':
        ...
        LADDER_MULTIPLIERS_LOCAL = [1.3, 1.6, 2.0, 2.5, 3.5, 5.0, 7.0, 10.0, 15.0, 25.0]
        coef = LADDER_MULTIPLIERS_LOCAL[min(step - 1, len(LADDER_MULTIPLIERS_LOCAL) - 1)]
        win = bet * coef
        async with pool.acquire() as conn:
            new_bal = await update_balance_and_xp(conn, tid, win)
            await conn.execute(

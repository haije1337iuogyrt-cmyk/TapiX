...
async def start(message: types.Message):
    ...
    if ref and user[5] == 0:  # balance == 0 значит новый игрок
        update_balance(ref, 50)
        await bot.send_message(ref, "🎉 Вам начислено 50 коинов за приглашённого!")

    WEBAPP_URL = "https://tapix-3k3q.onrender.com?v=3"  # <--- СБРОС КЭША
...
async def api_handler(request):
    ...
    if action == 'get_user':
        ...
        if user:
            return web.json_response({
                ...
                'referral_earned': 0,  # можно доработать
    ...
    if action == 'buy_boost':
        ...
        conn.commit()
        # Отключаем бустер через 5 минут
        asyncio.create_task(disable_boost(telegram_id, user[4] * 2))
        user = get_user(telegram_id)
        return web.json_response({'success': True, 'new_balance': user[2], 'per_sec': user[4]})

    if action == 'lottery_bet':
        # Упрощённо, для полноты можно добавить, но пока заглушка
        return web.json_response({'success': False, 'error': 'Лотерея в разработке'})

    if action == 'start_miner':
        # Заглушка
        return web.json_response({'success': False, 'error': 'Минёр в разработке'})

    if action == 'miner_lose' or action == 'miner_win' or action == 'miner_cashout':
        return web.json_response({'success': False, 'error': 'Минёр временно отключён'})
    ...
    if action == 'update_offline':
        earned = data.get('earned', 0)
        cursor.execute("UPDATE users SET offline_earned = offline_earned + ? WHERE telegram_id = ?", (earned, telegram_id))
        conn.commit()
        return web.json_response({'success': True})

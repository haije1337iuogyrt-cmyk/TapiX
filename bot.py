...
# ===== RATE LIMITING (не более 5 тапов в секунду) =====
tap_limits = {}  # telegram_id -> список времен тапов

# ===== ФОНДОВАЯ ЗАДАЧА ДЛЯ ПАССИВНОГО ДОХОДА =====
passive_task = None
...
async def init_db():
    ...
    async with db_pool.acquire() as conn:
        ...
        ''')
        # Таблицы для лотереи и минёра (позже добавим, пока заглушки)
        await conn.execute('''
...
async def update_top_cache():
    global top_cache
    while True:
        await asyncio.sleep(10)  # обновляем раз в 10 секунд
        try:
...
async def start(message: types.Message):
    ...
    WEBAPP_URL = "https://tapix-3k3q.onrender.com?v=6"
    ...
    if action == 'get_user':
        ...
        if user:
            return web.json_response({
                ...
                'referral_total': 0,  # позже посчитаем
                'referral_earned': 0,
    ...
    if action == 'buy_boost':
        ...
        async with db_pool.acquire() as conn:
            await conn.execute("UPDATE users SET balance = balance - 300, per_sec = per_sec * 2, boost_active = TRUE, boost_cooldown = $1 WHERE telegram_id = $2",
                               datetime.now() + datetime.timedelta(hours=1), telegram_id)
        updated = await get_user(telegram_id)
        # Отключаем бустер через 5 минут (фоном)

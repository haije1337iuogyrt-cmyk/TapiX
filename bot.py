...
CREATE TABLE IF NOT EXISTS ladder_sessions (
    telegram_id BIGINT PRIMARY KEY,
    bet DOUBLE PRECISION, stones INTEGER,
    step INTEGER DEFAULT 0,
    server_seed TEXT, seed_hash TEXT, nonce TEXT,
    created_at TIMESTAMPTZ DEFAULT now()
);
...
async def passive_income_loop():
    """
    Начисляем пассивный доход раз в 5 сек.
    last_passive_at — отметка последнего начисления, чтобы после перезапуска
    сервера не начислять двойное: считаем только реально прошедшее время.
    """
    # При старте — фиксируем момент, чтобы не компенсировать downtime
    async with pool.acquire() as conn:
        # Добавляем колонку если ещё нет
        try:
            await conn.execute(
                "ALTER TABLE users ADD COLUMN IF NOT EXISTS last_passive_at TIMESTAMPTZ DEFAULT now()"
            )
        except Exception:
            pass
        await conn.execute("UPDATE users SET last_passive_at=now() WHERE per_sec>0")

    while True:
        await asyncio.sleep(5)
        try:
            async with pool.acquire() as conn:
                # Начисляем ровно столько секунд, сколько прошло с last_passive_at,
                # но не больше 10 сек (защита от слишком долгого простоя сервера)
                await conn.execute("""
                    UPDATE users SET
                        balance = balance + per_sec * LEAST(EXTRACT(EPOCH FROM (now() - last_passive_at)), 10),
                        total_earned = total_earned + per_sec * LEAST(EXTRACT(EPOCH FROM (now() - last_passive_at)), 10),
                        last_passive_at = now()
                    WHERE per_sec > 0
                """)
        except Exception as e:
...
async def disable_boost(tid, orig_sec):
    """
    Ждём окончания буста и сбрасываем per_sec.
    Делаем деление — не полагаемся на переданный orig_sec из памяти,
    т.к. после перезапуска сервера in-memory значение пропадает.
    Boost_cooldown хранится в БД — используем его как источник истины.
    """
    await asyncio.sleep(305)  # небольшой запас
    try:
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT per_sec, boost_active, boost_cooldown FROM users WHERE telegram_id=$1", tid
            )
            if not row or not row['boost_active']:
                return  # уже сброшен
            # per_sec сейчас удвоен — делим пополам
            real_sec = float(row['per_sec']) / 2
            await conn.execute(
...
async def restore_boosts_after_restart():
    """
    Вызывается при старте: находит юзеров с активным бустом,
    чей boost_cooldown ещё не истёк — восстанавливает таймер отключения.
    """
    try:
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT telegram_id, per_sec FROM users "
                "WHERE boost_active=TRUE AND boost_cooldown > now()"
            )
        for row in rows:
            remaining = 300  # приближение, точное время в cooldown
            asyncio.create_task(_delayed_disable_boost(row['telegram_id'], remaining))
        # Сразу сбрасываем тех, у кого cooldown уже истёк, но boost_active ещё TRUE
        async with pool.acquire() as conn:
            await conn.execute("""
                UPDATE users SET
                    per_sec = per_sec / 2,
                    boost_active = FALSE
                WHERE boost_active = TRUE AND (boost_cooldown IS NULL OR boost_cooldown <= now())
            """)
    except Exception as e:
        print(f"restore_boosts error: {e}")

async def _delayed_disable_boost(tid, delay_sec):
    await asyncio.sleep(delay_sec)
    try:
        async with pool.acquire() as conn:
            row = await conn.fetchrow("SELECT per_sec, boost_active FROM users WHERE telegram_id=$1", tid)
            if row and row['boost_active']:
                await conn.execute(
                    "UPDATE users SET per_sec=$1, boost_active=FALSE WHERE telegram_id=$2",
                    float(row['per_sec']) / 2, tid
                )
    except Exception as e:
        print(f"_delayed_disable_boost error: {e}")
...
# ===== IDEMPOTENCY CACHE =====
# Храним результаты последних запросов: (tid, idem_key) -> (ts, response_body)
_idem_cache: dict = {}
_IDEM_TTL = 30  # секунд

def _idem_cleanup():
    now = time.time()
    stale = [k for k, (ts, _) in _idem_cache.items() if now - ts > _IDEM_TTL]
    for k in stale:
...
async def api_handler(request):
    ...
    # --- Idempotency: дедупликация повторных запросов от клиента ---
    idem_key = data.get('_idem')
    if idem_key and action in ('tap', 'lottery_bet', 'start_miner', 'miner_cashout',
                                'reveal_miner_cell', 'buy_shop', 'buy_boost',
                                'transfer', 'activate_promo',
                                'start_ladder', 'ladder_step', 'ladder_cashout'):
        cache_key = (tid, idem_key)
        _idem_cleanup()
        if cache_key in _idem_cache:
            _, cached = _idem_cache[cache_key]
    ...
    # ===== TAP (single) + TAP_BATCH =====
    if action == 'tap_batch':
        maybe_cleanup_limits()
        try:
            count = max(1, min(int(data.get('count', 1)), 20))  # не более 20 тапов за раз
        except (TypeError, ValueError):
            count = 1
        # Проверяем рейт-лимит (считаем как count тапов)
        ...
        async with pool.acquire() as conn:
            ...
            earn = float(u['per_tap']) * count
            row = await conn.fetchrow(
                "UPDATE users SET balance=balance+$1, total_earned=total_earned+$1, total_taps=total_taps+$3"
                " WHERE telegram_id=$2 RETURNING balance, total_taps, total_earned",
                earn, tid, count
            )
            bal = float(row['balance'])
            new_taps = int(row['total_taps'])
            if new_taps % 10 < count:  # пересекли кратное 10
                lvl, xp, xp_max = calc_xp(float(row['total_earned']))
                await conn.execute("UPDATE users SET level=$1,xp=$2,xp_max=$3 WHERE telegram_id=$4", lvl, xp, xp_max, tid)
        return web.json_response({'success': True, 'new_balance': bal})
    ...
    if action == 'tap':
        ...
        async with pool.acquire() as conn:
            ...
            # Одним запросом: обновляем и balance, и total_taps, и total_earned
            row = await conn.fetchrow(
            ...
            if new_taps % 10 == 0:
                lvl, xp, xp_max = calc_xp(float(row['total_earned']))
                await conn.execute("UPDATE users SET level=$1,xp=$2,xp_max=$3 WHERE telegram_id=$4", lvl, xp, xp_max, tid)
        resp = {'success': True, 'new_balance': bal}
        if idem_key:
            _idem_cache[(tid, idem_key)] = (time.time(), resp)
    ...
    if action == 'buy_shop':
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                # Проверяем баланс внутри транзакции с FOR UPDATE
                bal_check = await conn.fetchval(
                    "SELECT balance FROM users WHERE telegram_id=$1 FOR UPDATE", tid
                )
                if bal_check is None or float(bal_check) < price:
                    return web.json_response({'success': False, 'error': f'Нужно {price} ₿'})
                row = await conn.fetchrow(
        ...
        resp = {'success': True, 'new_balance': float(row['balance']), field: float(row[field])}
        if idem_key:
            _idem_cache[(tid, idem_key)] = (time.time(), resp)
        return web.json_response(resp)
    ...
    if action == 'buy_boost':
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                u = await conn.fetchrow(
                    "SELECT balance, boost_active, boost_cooldown, per_sec "
                    "FROM users WHERE telegram_id=$1 FOR UPDATE", tid
                )
                if not u: return web.json_response({'success': False, 'error': 'Not found'})
                ...
                if float(u['balance']) < 500:
                    return web.json_response({'success': False, 'error': 'Нужно 500 ₿'})
                row = await conn.fetchrow(
        ...
        resp = {'success': True, 'new_balance': float(row['balance']), 'per_sec': float(row['per_sec'])}
        if idem_key:
            _idem_cache[(tid, idem_key)] = (time.time(), resp)
        return web.json_response(resp)
    ...
    if action == 'miner_cashout':
        async with pool.acquire() as conn:
            async with conn.transaction():
                # FOR UPDATE — исключаем гонку: два одновременных cashout
                s = await conn.fetchrow(
                    "SELECT * FROM miner_sessions WHERE telegram_id=$1 FOR UPDATE", tid
                )
                if not s:
        ...
        resp = {'success': True, 'win_amount': win, 'new_balance': new_bal, 'bomb_positions': bomb_pos}
        if idem_key:
            _idem_cache[(tid, idem_key)] = (time.time(), resp)
        return web.json_response(resp)
    ...
    # ===== LADDER (ЛЕСЕНКА) =====
    # Сервер-авторитетная сессия: ставка списывается сразу, позиции камней
    # детерминированы через HMAC-SHA256(server_seed, nonce:step:counter) —
    # честность проверяема после раскрытия server_seed.
    # 4 клетки в строке, 1 или 2 камня из 4.
    # p(1 камень) = 3/4, p(2 камня) = 2/4. Множители растут как 1/p с house edge 0.97.
    LADDER_CELLS = 4
    LADDER_STEPS = 10
    LADDER_MIN_BET = 100
    LADDER_MAX_BET = 150_000
    LADDER_MULTS = {
        1: [1.29, 1.67, 2.16, 2.80, 3.62, 4.68, 6.05, 7.83, 10.12, 13.09],   # 3/4 шанс за ступень
        2: [1.94, 3.76, 7.30, 14.16, 27.48, 53.31, 103.42, 200.64, 389.24, 755.12],  # 2/4 шанс за ступень
    }

    def calc_ladder_mult(stones_count, step):
        """Множитель для числа камней и номера ступени (0-based, номер завершённой ступени)."""
        stones_count = max(1, min(2, stones_count))
        mults = LADDER_MULTS[stones_count]
        return mults[min(step, len(mults) - 1)]

    def ladder_stone_positions(server_seed, nonce, step, stones_count):
        """Детерминированно вывести позиции камней из server_seed+nonce+step через HMAC-SHA256."""
        positions = set()
        counter = 0
        while len(positions) < stones_count:
            digest = hmac.new(
                server_seed.encode(), f"{nonce}:{step}:{counter}".encode(), hashlib.sha256
            ).hexdigest()
            positions.add(int(digest[:8], 16) % LADDER_CELLS)
            counter += 1
        return sorted(positions)

    if action == 'start_ladder':
        try:
            bet = float(data.get('bet', 0))
            stones = int(data.get('stones', 1))
        except: return web.json_response({'success': False, 'error': 'Некорректные параметры'})
        if not LADDER_MIN_BET <= bet <= LADDER_MAX_BET:
            return web.json_response({'success': False, 'error': f'Ставка {LADDER_MIN_BET}–{LADDER_MAX_BET}'})
        if not 1 <= stones <= 2:
            return web.json_response({'success': False, 'error': 'Камней 1 или 2'})
        async with pool.acquire() as conn:
            existing = await conn.fetchrow("SELECT bet FROM ladder_sessions WHERE telegram_id=$1", tid)
            if existing:
                return web.json_response({'success': False, 'error': 'Игра уже идёт, забери выигрыш или дождись проигрыша'})
            async with conn.transaction():
                bal = await conn.fetchval("SELECT balance FROM users WHERE telegram_id=$1 FOR UPDATE", tid)
                ...
                await conn.execute(
                    "INSERT INTO ladder_sessions(telegram_id,bet,stones,step,server_seed,seed_hash,nonce)"
                    " VALUES($1,$2,$3,0,$4,$5,$6) ON CONFLICT(telegram_id) DO UPDATE"
                    " SET bet=$2,stones=$3,step=0,server_seed=$4,seed_hash=$5,nonce=$6,created_at=now()",
                    tid, bet, stones, server_seed, seed_hash, nonce
                )
        return web.json_response({
            'success': True, 'new_balance': float(new_bal), 'seed_hash': seed_hash,
            'cells': LADDER_CELLS, 'steps': LADDER_STEPS
        })

    if action == 'get_ladder_session':
        async with pool.acquire() as conn:
            s = await conn.fetchrow("SELECT * FROM ladder_sessions WHERE telegram_id=$1", tid)
        if not s:
            return web.json_response({'success': True, 'active': False})
        return web.json_response({
            'success': True, 'active': True,
            'bet': float(s['bet']), 'stones': s['stones'], 'step': s['step'],
            'seed_hash': s['seed_hash'],
            'coef': calc_ladder_mult(s['stones'], s['step'] - 1) if s['step'] > 0 else 1.0,
            'cells': LADDER_CELLS, 'steps': LADDER_STEPS,
        })
    ...
    if action == 'ladder_step':
        ...
        except: return web.json_response({'success': False, 'error': 'Некорректная клетка'})
        if not 0 <= cell < LADDER_CELLS:
            return web.json_response({'success': False, 'error': 'Некорректная клетка'})
        async with pool.acquire() as conn:
            async with conn.transaction():
                s = await conn.fetchrow("SELECT * FROM ladder_sessions WHERE telegram_id=$1 FOR UPDATE", tid)
                if not s:
                    return web.json_response({'success': False, 'error': 'Нет активной игры — начни новую'})
                step = s['step']
                stones = s['stones']
                bet = float(s['bet'])
                stone_positions = ladder_stone_positions(s['server_seed'], s['nonce'], step, stones)
                if cell in stone_positions:
                    await conn.execute("DELETE FROM ladder_sessions WHERE telegram_id=$1", tid)
                    await conn.execute(
                        "INSERT INTO game_history(telegram_id,game,result,amount) VALUES($1,'ladder','lose',$2)",
                        tid, -bet
                    )
                    return web.json_response({
                        'success': True, 'result': 'stone', 'stone_positions': stone_positions,
                        'server_seed': s['server_seed'], 'seed_hash': s['seed_hash'], 'nonce': s['nonce']
                    })
                next_step = step + 1
                complete = next_step >= LADDER_STEPS
                if complete:
                    coef = calc_ladder_mult(stones, LADDER_STEPS - 1)
                    win = bet * coef
                    new_bal = await update_balance_and_xp(conn, tid, win)
                    await conn.execute("DELETE FROM ladder_sessions WHERE telegram_id=$1", tid)
                    await conn.execute(
                    ...
                    return web.json_response({
                        'success': True, 'result': 'safe', 'stone_positions': stone_positions,
                        'complete': True, 'win_amount': win, 'new_balance': new_bal,
                        'server_seed': s['server_seed'], 'seed_hash': s['seed_hash'], 'nonce': s['nonce']
                    })
                await conn.execute("UPDATE ladder_sessions SET step=$1 WHERE telegram_id=$2", next_step, tid)
                coef = calc_ladder_mult(stones, next_step - 1)
        return web.json_response({
            'success': True, 'result': 'safe', 'stone_positions': stone_positions,
            'complete': False, 'coef': coef, 'step': next_step
        })
    ...
    if action == 'ladder_cashout':
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                if not s:
                    return web.json_response({'success': False, 'error': 'Нет активной игры'})
                if s['step'] <= 0:
                    return web.json_response({'success': False, 'error': 'Поднимись хотя бы на одну ступень'})
                coef = calc_ladder_mult(s['stones'], s['step'] - 1)
                win = float(s['bet']) * coef
                new_bal = await update_balance_and_xp(conn, tid, win)
                await conn.execute("DELETE FROM ladder_sessions WHERE telegram_id=$1", tid)
                await conn.execute(
        ...
        resp = {
            'success': True, 'win_amount': win, 'new_balance': new_bal,
            'server_seed': s['server_seed'], 'seed_hash': s['seed_hash'], 'nonce': s['nonce']
        }
        if idem_key:
            _idem_cache[(tid, idem_key)] = (time.time(), resp)
...
async def reset_handler(request):
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute("DELETE FROM ladder_sessions")
...
async def main():
    await init_db()
    await restore_boosts_after_restart()  # восстанавливаем бусты после рестарта

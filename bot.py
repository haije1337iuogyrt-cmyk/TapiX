...
# ===== НАСТРОЙКИ ИГР =====
LOTTERY_BET_SECONDS = 300     # 5 минут приём ставок
...
async def keep_alive_loop():
    ...
    import aiohttp
    await asyncio.sleep(60)  # ждём старта сервера
    while True:
        try:
            async with aiohttp.ClientSession() as session:
                await session.get(WEBAPP_URL, timeout=aiohttp.ClientTimeout(total=10))
...
async def reset_all_cmd(message: types.Message):
    """Команда для сброса прогресса всех игроков (только для администратора)."""
    async with pool.acquire() as conn:
        await conn.execute("""
            UPDATE users SET balance=0, total_earned=0, total_taps=0, level=1, xp=0, xp_max=800,
            per_tap=0.001, per_sec=0.00005,
            shop_tap0=0, shop_tap1=0, shop_tap2=0, shop_tap3=0, shop_tap4=0,
            shop_sec0=0, shop_sec1=0, shop_sec2=0, shop_sec3=0, shop_sec4=0,
            promo_used=FALSE, boost_active=FALSE, boost_cooldown=NULL
        """)
    ...
    await message.answer("✅ Прогресс всех игроков сброшен!")


@dp.message(Command("addbalance"))
async def add_balance_cmd(message: types.Message):
    """Секретная команда для тестирования: /addbalance 10000"""
    parts = message.text.split()
    if len(parts) < 2:
        await message.answer("Использование: /addbalance 10000")
        return
    try:
    ...
    await message.answer(f"✅ Начислено {amount:.0f} ₿. Новый баланс: {new_balance:.2f} ₿")
...
async def start(message: types.Message):
    ...
    if len(message.text.split()) > 1:
        ...
        async with pool.acquire() as conn:
            ref_row = await conn.fetchrow("SELECT id, telegram_id FROM users WHERE referral_code = $1", ref_code)
        if ref_row and ref_row['telegram_id'] != message.from_user.id:
            ref = ref_row['id']
    ...
    # Начисляем бонус только новому пользователю пришедшему по рефералке
    if is_new and ref:
        async with pool.acquire() as conn:
        ...
        if ref_telegram_row:
            ...
            try:
                await bot.send_message(ref_telegram_row['telegram_id'], "🎉 +50 ₿ — твой друг присоединился к TapiX!")
            except Exception:
                pass
...
async def show_referral(callback: types.CallbackQuery):
    ...
    if user:
        ...
        link = f"https://t.me/taplx_bot?start={code}"
...
async def api_handler(request):
    ...
    if action == 'get_user':
        ...
        # Реальная статистика рефералов
        async with pool.acquire() as conn:
            ref_count = await conn.fetchval(
                "SELECT COUNT(*) FROM users WHERE referred_by = $1", user['id']
            ) or 0
            ref_earned = ref_count * 50.0
        result['referral_total'] = ref_count
        result['referral_earned'] = ref_earned
    ...
    if action == 'transfer':
        ...
        to_username = (data.get('to_username') or '').lstrip('@').strip()
        try:
            amount = float(data.get('amount', 0))
        except (TypeError, ValueError):
            return web.json_response({'success': False, 'error': 'Некорректная сумма'})
        if amount < 50:
            return web.json_response({'success': False, 'error': 'Минимальная сумма перевода — 50 ₿'})
        fee = amount * 0.05
        total = amount + fee

        async with pool.acquire() as conn:
            to_user = await conn.fetchrow(
                "SELECT telegram_id FROM users WHERE LOWER(username) = LOWER($1)", to_username
            )
            if not to_user:
                return web.json_response({'success': False, 'error': 'Игрок не найден — убедись что он зарегистрирован в игре'})
    ...
    if action == 'sync_progress':
        try:
            ...
            xp_max = float(data.get('xp_max', 800))
            total_earned = float(data.get('total_earned', 0))
        except (TypeError, ValueError):
            return web.json_response({'success': False, 'error': 'Bad data'})
        async with pool.acquire() as conn:
            await conn.execute(
                "UPDATE users SET xp=$1, level=$2, xp_max=$3, total_earned=GREATEST(total_earned,$4) WHERE telegram_id=$5",
                xp, level, xp_max, total_earned, telegram_id
            )
    ...
    if action == 'get_game_history':
        ...
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT game, result, amount, date FROM game_history WHERE telegram_id = $1 AND game = $2 ORDER BY date DESC LIMIT 50",
                telegram_id, game
            )
        history = [{'game': r['game'], 'result': r['result'], 'amount': float(r['amount']), 'date': r['date'].isoformat()} for r in rows]
    ...
    if action == 'activate_promo':
        ...
        if code == 'WELCOME' and not user['promo_used']:
            async with pool.acquire() as conn:
                new_balance = await conn.fetchval(
                    "UPDATE users SET balance = balance + 1000, promo_used = TRUE WHERE telegram_id = $1 RETURNING balance",
                    telegram_id
                )
            return web.json_response({'success': True, 'new_balance': float(new_balance), 'bonus': 1000})
